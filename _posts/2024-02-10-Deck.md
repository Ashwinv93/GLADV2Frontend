...
function saveDeck() {
    const deckName = document.getElementById('deck-name').value; // Assuming you have an input field for the deck name
    const deckData = {
        name: deckName,
        cards: deck // Assuming 'deck' is an array of card IDs you've built up
    };
    fetch('http://127.0.0.1:8086/saveDeck', {
        method: 'POST',
        headers: {
