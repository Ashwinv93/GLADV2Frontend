...
function saveDeck() {
    const deckName = document.getElementById('deck-name').value.trim(); // Get the deck name from an input field
    if (!deckName) {
        alert('Please enter a name for your deck.');
        return;
    }
    // Find card names by their IDs in the 'deck' array
    const cardNames = deck.map(cardId => {
        const card = cardData.items.find(item => item.id === cardId);
        return card ? card.name : null; // Return null if card is not found (shouldn't happen in practice)
    }).filter(name => name !== null); // Filter out any nulls (just in case)
    ...
    const deckData = {
        title: deckName, // Use 'title' or another key as needed by your backend
        cards: cardNames // An array of card names
    };
    // Send the data to the backend
    fetch('http://127.0.0.1:8006/saveDeck', {
        method: 'POST',
        headers: {
